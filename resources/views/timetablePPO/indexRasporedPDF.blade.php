    ...
    <body>
        @foreach($rasporedSve[2] as $index => $byWeeksOne)
            @if(isset($rasporedSve))
                <span style="font-size: 1.2em; font-weight: bold">POSEBNI PROGRAM OBRAZOVANJA - {{Str::upper($rasporedSve[6])}} {{Str::upper($rasporedSve[8])}}</span><hr/>
                    ...
                    <tbody>
                        @foreach($rasporedSve[1][$index] as $time) 
                                <tr>
                                    ...
                                    @foreach($rasporedSve[0] as $index2 => $day)
